...
async function scrapeWithReservationId(
  page: Page,
  reservation: any
) {
  try {
        ...
        try {
          ...
          const clicked = await page.evaluate((searchId) => {
            ...
            for (const row of rows) {
              ...
              if (idElement && idElement.textContent && idElement.textContent.includes(searchId)) {
                const link = row.querySelector('.property-cell__property-name a') as HTMLAnchorElement;
                if (link) {
                  link.click();
                  return true;
                }
      ...
      try {
        ...
        try {
          ...
          const clicked = await page.evaluate(() => {
            ...
            const logoLink = document.querySelector('header.tpg-navigation__header a.tpg-navigation__logo_container') as HTMLElement;
            if (logoLink) {
              logoLink.click();
              return true;
            }
          ...
          if (!clicked) {
            ...
            const href = await page.evaluate(() => {
              const logoLink = document.querySelector('header.tpg-navigation__header a.tpg-navigation__logo_container') as HTMLAnchorElement;
